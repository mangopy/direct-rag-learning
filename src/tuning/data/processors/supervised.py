...
def preprocess_supervised_dataset(
    ...
    keys = examples.keys()
    model_inputs.update({k: examples[k] for k in keys if k not in pad_features and (type(examples[k][0]) is int or type(examples[k][0] is float))})
    # print([type(examples[k][0]) for k in keys])
    return model_inputs
